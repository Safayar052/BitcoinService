...
public class ButtonService {
	...
	public String createButton() throws IOException {
		...
		buttonParams
				.setCallbackUrl("http://www.example.com/my_custom_button_callback");
		buttonParams
				.setSuccessUrl("http://www.vayama.com/abc.jsp?result=success");
		buttonParams
				.setCancelUrl("http://www.vayama.com/abc.jsp?result=failure");
		...
		String message = nonce + buttonsUrl.toString()
				+ (body != null ? body : "");

		Mac mac = null;
		try {
			mac = Mac.getInstance("HmacSHA256");
			mac.init(new SecretKeySpec(COINBASE_API_SECRET.getBytes(),
					"HmacSHA256"));
		} catch (Exception t) {
			t.printStackTrace();
		}

		String signature = new String(Hex.encodeHex(mac.doFinal(message
				.getBytes())));
